...
import { addContract, editContract, contractCleanError, resetContractsFormSaved, resetContractsForm, addContractSaved, addCustomerReseachedSaved, addVehicleReseachedSaved, resetUuidContractSuccess } from "../../../store/modules/contracts/actions"
import api from "../../../services/api"
import * as Icon from "react-feather"
import classnames from 'classnames'
import "react-toastify/dist/ReactToastify.css"
import CardText from 'reactstrap/lib/CardText'
import { AlertError, AlertErrorRedFull } from "../components/alerts"
import '../../styles/fullFormstyles.scss'
import { useHistory } from "react-router-dom"
import { addTab } from "../../../store/modules/navbar/actions"
import { SweetAlertQuestion, SweetAlertSuccess, SweetAlertSuccessPDF } from "../components/sweetAlertMessage"
import { resetCustomerFormSaved } from "../../../store/modules/customers/actions"
import generatePdf from "./generatePDF"

const FullForm = props => {
  const dispatch = useDispatch()
  const contractSaved = useSelector(state => state.contracts.contractSaved)
  const customerSaved = useSelector(state => state.contracts.customerReseachedSaved)
  const vehicleSaved = useSelector(state => state.contracts.vehicleReseachedSaved)
  const uuidContractSuccess = useSelector(state => state.contracts.uuidContractSuccess)
  const [contract, setContract] = useState(contractSaved ?? {})
  ...
  const [customerResearched, setCustomerResearched] = useState(customerSaved ?? [])
  const [incomesResearched, setIncomesResearched] = useState([])
  const [contactsResearched, setContactssResearched] = useState([])
  const [bankingReferencesResearched, setBankingReferencesResearched] = useState([])
  const [vehicleResearched, setVehicleResearched] = useState(vehicleSaved ?? [])
  const formSuccess = useSelector(state => state.contracts.formSuccess)
  ...
  useEffect(() => {
    dispatch(addContractSaved(contract))
    dispatch(addCustomerReseachedSaved(customerResearched))
    dispatch(addVehicleReseachedSaved(vehicleResearched))
  })
  ...
  useEffect(() => {
    if (formSuccess) {
      SweetAlertSuccessPDF("Finalizado", "O contrato foi cadastrado com sucesso!", "success", function () { closeForm() }, function () {
        generatePdf(uuidContractSuccess)
        closeForm()
      })
      dispatch(resetContractsForm())
      dispatch(resetContractsFormSaved())
      dispatch(resetUuidContractSuccess())
    }
  }, [formSuccess])
  ...
  const handleDataCustomerName = (search) => {
    api.get(`/customers/name/${search}`).then(response => {
      ...
      setIncomesResearched(result.incomes ?? [])
      setContactssResearched(result.contacts ?? [])
      setBankingReferencesResearched(result.banking_references ?? [])
    }).catch(err => {
    })
  }
  ...
  const saveContract = async () => {
    ...
    dispatch(addContract(contractToSave))
  }
      ...
      <Form>
        ...
        <Row>
          ...
          <Col md='6' sm='4'>
            <Card>
              <CardBody className='pt-0'>
                {vehicleResearched.uuid ? <CardText>
                  <div className='expandable-content p-2'>
                    <p>
