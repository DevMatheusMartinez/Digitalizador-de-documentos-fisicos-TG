...
import { addVehicleEditSaved, editVehicle, resetVehicleForm, resetVehicleFormEdit } from "../../../store/modules/vehicles/actions"
...
const FullForm = props => {
    ...
    const vehicleSaved = useSelector(state => state.vehicles.vehicleEditSaved)
    const tabs = useSelector(state => state.navbar.tabs)
    const [vehicle, setVehicle] = useState(vehicleSaved.length !== 0 ? vehicleSaved : props.location.state.vehicleDataView)
    ...
    const formSuccess = useSelector(state => state.vehicles.formSuccessEdit)
    const errors = useSelector(state => state.vehicles.errorsEdit)
    ...
    useEffect(() => {
        dispatch(addVehicleEditSaved(vehicle))
    })
    ...
    const saveVehicle = async () => {
        ...
        dispatch(editVehicle(vehicleToSave))
    }
    ...
    useEffect(() => {
        setVehicle({ ...vehicle, renavam: addZeroesRenavam(vehicle.renavam, 11), owner_doc: applyMaskToCpfCnpj(vehicle.owner_doc) })
    }, [])
