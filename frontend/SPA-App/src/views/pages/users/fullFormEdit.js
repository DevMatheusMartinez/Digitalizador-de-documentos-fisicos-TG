...
import { addAddressSavedEdit, addContactSavedEdit, addPermissionsEditSaved, addUserSavedEdit, editUser, resetUserForm, resetUserFormEdit, resetUserFormEditSaved, userCleanError } from "../../../store/modules/users/actions"
import { CheckboxForm, InputForm } from "../components/componentsForm"
...
const FullFormEdit = props => {
  const dispatch = useDispatch()
  const tabs = useSelector(state => state.navbar.tabs)
  const userSaved = useSelector(state => state.users.userSavedEdit)
  const contactSaved = useSelector(state => state.users.contactSavedEdit)
  const addressSaved = useSelector(state => state.users.addressSavedEdit)
  const permissionsSaved = useSelector(state => state.users.permissionsEditSaved)
  const [user, setUser] = useState(userSaved.length !== 0 ? userSaved : props.location.state.userDataView)
  const [userPermissions, setUserPermissions] = useState(permissionsSaved.length !== 0 ? permissionsSaved : user.permissions)
  const [address, setAddress] = useState(addressSaved.length !== 0 ? addressSaved : user.address ?? [])
  const [active, setActive] = useState("personalData")
  const errors = useSelector(state => state.users.errorsEdit)
  const formSuccess = useSelector(state => state.users.formSuccessEdit)
  const [inputContacts, setInputContacts] = useState(contactSaved.length !== 0 ? contactSaved : user.contacts)
  const [permissionsUuid, setPermissionsUuid] = useState([])
  const history = useHistory()

  useEffect(() => {
    dispatch(addUserSavedEdit(user))
    dispatch(addContactSavedEdit(inputContacts))
    dispatch(addAddressSavedEdit(address))
    dispatch(addPermissionsEditSaved(userPermissions))
  })
