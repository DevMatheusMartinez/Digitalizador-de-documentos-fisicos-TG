...
import { addAddressSaved, addBankSaved, addConjugeSaved, addContactSaved, addCustomer, addCustomerSaved, addIncomesSaved, editCustomer, resetCustomerForm, resetCustomerFormSaved } from "../../../store/modules/customers/actions"
...
const FullForm = props => {
    ...
    const customerSaved = useSelector(state => state.customers.customerSaved)
    const conjugeSaved = useSelector(state => state.customers.conjugeSaved)
    const addressSaved = useSelector(state => state.customers.addressSaved)
    const incomeSaved = useSelector(state => state.customers.incomeSaved)
    const contactSaved = useSelector(state => state.customers.contactSaved)
    const bankSaved = useSelector(state => state.customers.bankSaved)
    const [customer, setCustomer] = useState(customerSaved ?? {})
    ...
    const formSuccess = useSelector(state => state.customers.formSuccess)
    const customerRegistered = useSelector(state => state.customers.customerRegistered)
    ...
    const [conjuge, setConjuge] = useState(conjugeSaved ?? {})
    const [tabConjuge, setTabConjuge] = useState(customer.spouse !== null)
    const [inputIncomes, setInputIncomes] = useState(incomeSaved ?? [])
    const [inputBanks, setInputBanks] = useState(bankSaved ?? [])
    ...
    useEffect(() => {
        dispatch(addCustomerSaved(customer))
        dispatch(addConjugeSaved(conjuge))
        dispatch(addIncomesSaved(inputIncomes))
        dispatch(addContactSaved(inputContacts))
        dispatch(addAddressSaved(address))
        dispatch(addBankSaved(inputBanks))
    })

    const handleCurrentDataView = () => {
        api.get(`/customers/${customerRegistered.uuid}`).then(response => {
            props.openView(response.data.data)
        }).catch(err => {
        })
    }
    ...
    useEffect(() => {
        if (formSuccess) {
            SweetAlertSuccess("Finalizado", "O cliente foi cadastrado com sucesso!", "success", function () {
                closeForm()
            }
            )
    ...
    useEffect(() => {
        ...
        setErrorBank(false)
        if (errors['rg'] !== undefined || 
            errors['birthday'] !== undefined || 
            errors['cpf_cnpj'] !== undefined || 
            errors['name'] !== undefined || 
            errors['contact'] !== undefined ||
            errors['commercial_phone'] !== undefined ||
            errors['home_phone'] !== undefined
            ) {
                    ...
                    <TabContent activeTab={active}>
                        <TabPane tabId="personalData">
                            <FormCustomer
                                ...
                                fieldCommercialPhone="commercial_phone" idCommercialPhone="data-commercial_phone" valueCommercialPhone={customer.commercial_phone ?? ''} nameCommercialPhone="commercial_phone" functionCommercialPhone={e => setCustomer({ ...customer, commercial_phone: applyMaskToPhone(e.target.value) })}
                                fieldHomePhone="home_phone" idHomePhone="data-home_phone" valueHomePhone={customer.home_phone ?? ''} nameHomePhone="home_phone" functionHomePhone={e => setCustomer({ ...customer, home_phone: applyMaskToPhone(e.target.value) })}
                        ...
                        <TabPane tabId="spouseData">
                            <FormCustomer tabId="spouseData" errors={errors} customer={conjuge}
                                ...
                                fieldCommercialPhone="spouse_commercial_phone" idCommercialPhone="data-spouse_commercialPhone" valueCommercialPhone={conjuge.spouse_commercial_phone ?? ''} nameCommercialPhone="spouse_commercial_phone" functionCommercialPhone={e => setConjuge({ ...conjuge, spouse_commercial_phone: applyMaskToPhone(e.target.value) })}
                                fieldHomePhone="spouse_home_phone" idCommercialPhone="data-spouse_home_phone" valueHomePhone={conjuge.spouse_home_phone ?? ''} nameHomePhone="spouse_home_phone" functionHomePhone={e => setConjuge({ ...conjuge, spouse_home_phone: applyMaskToPhone(e.target.value) })}
