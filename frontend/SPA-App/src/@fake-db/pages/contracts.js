...
// GET DATA
mock.onGet("/api/contracts/initial-data").reply(response => {
  return [200, contracts]
})

mock.onGet("/api/contracts/data").reply(response => {
  let { page, perPage } = response
  let totalPages = Math.ceil(contracts.length / perPage)
  if (page !== undefined && perPage !== undefined) {
    let calculatedPage = (page - 1) * perPage
    let calculatedPerPage = page * perPage
    return [
      200,
      { data: contracts.slice(calculatedPage, calculatedPerPage), totalPages }
    ]
  } else {
    return [
      200,
      { data: contracts.slice(0, 4), totalPages: Math.ceil(contracts.length / 4) }
    ]
  }
})

// UPDATE DATA
mock.onPost("/api/contracts/update-data").reply(request => {
...
// Add DATA
mock.onPost("/api/contracts/add-data").reply(request => {
  let data = JSON.parse(request.data).obj
  let highestId = Math.max.apply(
    Math,
    contracts.map(i => i.id)
  )
  contracts.unshift({
    ...data,
    id: highestId + 1,
...
// DELETE DATA
mock.onPost("/api/contracts/delete-data").reply(request => {
  let data = JSON.parse(request.data).obj
  let index = contracts.findIndex(item => item.id === data.id)
  contracts.splice(index, 1)
  return [200]
})

// DELETE SELECTED DATA
mock.onPost("/api/contracts/delete-selected").reply(request => {
  let data = JSON.parse(request.data).arr
  let reducedArray
  ;[contracts, data].reduce((a, b) => {
    let c = b.map(j => j.id)
    return (reducedArray = a.filter(i => !c.includes(i.id)))
  })
  contracts = reducedArray
  return [200]
})
